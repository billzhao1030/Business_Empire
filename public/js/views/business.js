...
export default {
    ...
    return `<div class="biz-card" data-id="${b.id}">
      ...
      <div class="biz-body">
        <div class="biz-stats" style="grid-template-columns:repeat(5,1fr)">
          ...
          <div class="bs"><label>${t('biz.prosperity')}</label><b class="${(s.prosperity[b.cityId] || 1) >= 1 ? 'up' : 'down'}">${((s.prosperity[b.cityId] || 1) * 100).toFixed(0)}%</b></div>
        </div>
  ...
  openNew(app) {
    ...
    const render = el => {
      ...
      const rev = def.rev * city.revMult, opc = def.opc * city.revMult;
      const net = rev - opc;
      ...
      <div class="opt-grid" style="max-height:230px;overflow:auto;margin-bottom:16px">
        ${cat.biz.map(x => {
          const c = Math.round(x.cost * city.costMult);
